...
export default function AdminPage() {
  ...
  async function handleCreate() {
    ...
    if (!hasAnyText(createFields) && !hasMedia) {
      setErrorText("Isi minimal 1 title/description atau upload media.");
      return;
    }
    ...
    try {
      ...
      setErrorText(e?.message || "Create error");
    } finally {
      setPhase("idle");
    }
  }
  ...
  async function handleUpdate() {
    ...
    // minimal: boleh update text saja, tapi jangan izinkan benar-benar kosong semua kalau memang mau
    if (!hasAnyText(editFields) && !replaceMedia) {
      // masih boleh, tapi biasanya tidak berguna
      // kamu bisa hapus guard ini kalau mau memperbolehkan update kosong.
      setErrorText(
        "Isi minimal 1 title/description atau upload media replacement.",
      );
      return;
    }

    try {
      let payload: any = { fields: editFields };

      // kalau ada replacement media, upload dulu lalu update media_* di DB
      if (replaceMedia) {
        ...
        payload.mediaType = up.mediaType; // images/video
      ...
      setEditing(updated); // keep drawer data fresh
      setEditImages(null);
      setEditVideo(null);
      ...
      setErrorText(e?.message || "Update error");
    } finally {
      setPhase("idle");
    }
  }
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      setErrorText(e?.message || "Delete error");
    } finally {
      setPhase("idle");
    }
  }
  ...
  async function movePost(postId: string, direction: "up" | "down") {
    ...
    try {
      ...
      setErrorText(e?.message || "Reorder error");
      await refreshList();
    } finally {
      setPhase("idle");
    }
    ...
    <main className="space-y-6">
      ...
      <Card>
        <div className="flex items-center justify-between">
          ...
          <div className="text-xs text-zinc-500">Media optional</div>
        </div>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          ...
          <div className="grid gap-3">
            <AccordionSection
              title="Donasi (Infak / Wakaf / Zakat)"
              subtitle="Title + nominal rupiah + opsi target + progress"
              defaultOpen={false}
            >
              ...
              <div className="mt-3 text-xs text-zinc-500">
                Nominal = total terkumpul saat ini. Jika Target aktif, progress
                = nominal/target.
              </div>
            </AccordionSection>
            ...
            <AccordionSection
              title="Media"
              subtitle="Images slider / Video autoplay (optional)"
              defaultOpen={false}
            >
              <div className="grid gap-3">
                  ...
                  <div className="grid gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setCreateVideo(null); // bersihin biar gak nyangkut
                      }}
                      className="text-sm text-zinc-700 disabled:opacity-60"
                    />
                    <div className="text-xs text-zinc-500">Boleh kosong.</div>
                  </div>
                ) : isCreateVideo ? (
                  <div className="grid gap-2">
                    <label className="text-sm font-medium text-zinc-800">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setCreateImages(null); // bersihin biar gak nyangkut
                      }}
                      className="text-sm text-zinc-700 disabled:opacity-60"
                    />
                    <div className="text-xs text-zinc-500">Boleh kosong.</div>
            ...
            <button
              ...
              disabled={isBusy}
              className="mt-1 fixed bottom-4 right-4 inline-flex items-center justify-center gap-2 rounded-xl bg-zinc-900 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-70"
            >
          ...
          {/* RIGHT: Fields accordion */}
          <div className="grid gap-3">
            <AccordionSection
              title="Fields"
              subtitle="Title & Description (blok 1–4 di create)"
              defaultOpen={false}
            >
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const removedIndex = createVisibleBlocks; // block yang dihapus = paling akhir
                    setCreateFields((prev) => ({
                      ...prev,
                      [`title_${removedIndex}`]: "",
              ...
              <div className="mt-3 text-xs text-zinc-500">
                (Di create form ini ditampilkan 4 block dulu supaya ringkas.
                Edit mode lengkap 1–10.)
              </div>
            </AccordionSection>
          </div>
        </div>
            ...
            <div className="sticky top-0 border-b border-zinc-200 bg-white p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  onClick={closeEdit}
                  disabled={isBusy}
                  className="rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm font-medium text-zinc-900 hover:bg-zinc-50 disabled:opacity-70"
                >
                  Close
                </button>
              </div>
              ...
              <div className="mt-3 flex gap-2">
                <button
                  ...
                  disabled={isBusy}
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-zinc-900 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-800 disabled:opacity-70"
                >
                  {phase === "uploading_update" || phase === "updating" ? (
                    <>
                      <Spinner className="border-zinc-600 border-t-white" />
                      {phase === "uploading_update"
                        ? "Uploading..."
                ...
                <button
                  ...
                  disabled={isBusy}
                  className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-medium text-rose-800 hover:bg-rose-100 disabled:opacity-70"
                >
                  Delete
                </button>
              </div>
              ...
              {statusText || errorText ? (
                <div className="mt-3 rounded-xl border border-zinc-200 bg-zinc-50 p-3 text-sm text-zinc-700">
                  {errorText ? (
                    <div className="text-rose-700">{errorText}</div>
                  ) : (
                    <div>{statusText}</div>
                  )}
                </div>
            ...
            <div className="p-4 space-y-5">
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white p-4">
                <div className="rounded-2xl border border-zinc-200 bg-white p-4">
                  ...
                  <div className="mt-1 text-xs text-zinc-500">
                    Berlaku hanya untuk Media Type Pop-up.
                  </div>
                  ...
                  <div className="mt-4 grid gap-3">
                    <label className="flex items-center gap-2 text-sm text-zinc-700">
                      <input
                        type="checkbox"
                        checked={editFields["popup_enabled"] === "true"}
                        disabled={isBusy}
                        onChange={(e) =>
                          setEditFields((prev) => ({
                            ...prev,
                            popup_enabled: String(e.target.checked),
                          }))
                        }
                        className="h-4 w-4 rounded border-zinc-300"
                      />
                    ...
                    <div className="grid gap-2">
                      ...
                      <input
                        ...
                        inputMode="numeric"
                        disabled={isBusy}
                        value={editFields["popup_slider_interval_ms"] || "4000"}
                        onChange={(e) =>
                          setEditFields((prev) => ({
                            ...prev,
                            popup_slider_interval_ms: e.target.value,
                          }))
                        }
                    ...
                    {/* <button
                        ...
                        onClick={() =>
                          setEditFields((prev) => ({
                            ...prev,
                            popup_enabled: "true",
                            popup_trigger_ts: String(Date.now()),
                          }))
                        }
                        className="inline-flex items-center justify-center rounded-xl bg-zinc-900 px-3 py-2 text-sm font-medium text-white hover:bg-zinc-800 disabled:opacity-70"
                      >
                        Trigger Pop-up Now
                      </button>
                      ...
                      <button
                        ...
                        className="inline-flex items-center justify-center rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm font-medium text-zinc-900 hover:bg-zinc-50 disabled:opacity-70"
                      >
                        Disable Pop-up
                      </button>
                    </div> */}
                    ...
                    <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
                      <label className="flex items-center gap-2 text-sm text-zinc-700">
                        <input
                          type="checkbox"
                          checked={editFields["popup_cycle_enabled"] === "true"}
                          disabled={isBusy}
                          onChange={(e) =>
                            setEditFields((prev) => ({
                              ...prev,
                              popup_cycle_enabled: String(e.target.checked),
                            }))
                          }
                          className="h-4 w-4 rounded border-zinc-300"
                        />
                        Enable Cycle Mode (tampil-sembunyi berulang)
                      </label>
                      ...
                      <div className="mt-3 grid gap-3 sm:grid-cols-2">
                        <div className="grid gap-2">
                          ...
                          <input
                            ...
                            inputMode="numeric"
                            disabled={isBusy}
                            value={editFields["popup_cycle_show_ms"] || "10000"}
                            onChange={(e) =>
                              setEditFields((prev) => ({
                                ...prev,
                                popup_cycle_show_ms: e.target.value,
                              }))
                            }
                            className="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:border-zinc-400 disabled:opacity-60"
                          />
                        ...
                        <div className="grid gap-2">
                          ...
                          <input
                            ...
                            inputMode="numeric"
                            disabled={isBusy}
                            value={editFields["popup_cycle_hide_ms"] || "30000"}
                            onChange={(e) =>
                              setEditFields((prev) => ({
                                ...prev,
                                popup_cycle_hide_ms: e.target.value,
                              }))
                            }
                            className="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:border-zinc-400 disabled:opacity-60"
                          />
                      ...
                      <div className="mt-2 text-xs text-zinc-500">
                        Contoh: show 10000 (10 detik), hide 30000 (30 detik).
                      </div>
                    </div>
                  </div>
                </div>

                <div className="text-sm font-semibold text-zinc-900 mt-5">
                  Media (optional)
                </div>
                <div className="mt-1 text-xs text-zinc-500">
                  Kalau kamu upload file baru di sini, media lama akan
                  tergantikan. Kalau kosong, media tetap.
                </div>
                ...
                <div className="mt-4 grid gap-3">
                  <div className="grid gap-2">
                    ...
                    >
                      <option value="images">Images Slider</option>
                      <option value="video">Video Autoplay</option>
                      <option value="popup_images">Popup Slider</option>
                      <option value="popup_video">Popup Video</option>
                    </select>
                  </div>

                  {editMediaType === "images" ||
                  editMediaType === "popup_images" ? (
                    <div className="grid gap-2">
                      <label className="text-sm font-medium text-zinc-800">
                        Replace Images
                      </label>
                      ...
                      <div className="text-xs text-zinc-500">
                        Current:{" "}
                        {Array.isArray(editing.media_urls) &&
                        editing.media_urls.length
                          ? `${editing.media_urls.length} files`
                          : "none"}
                      </div>
                    </div>
                  ) : (
              ...
              {/* Donation (Infak/Wakaf/Zakat) */}
              <div className="rounded-2xl border border-zinc-200 bg-white p-4">
                <div className="text-sm font-semibold text-zinc-900">
                  Donasi (Infak / Wakaf / Zakat)
                </div>
                <div className="mt-1 text-xs text-zinc-500">
                  Edit nominal/target. Update akan push realtime ke frontpage.
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setEditVisibleBlocks((v) => Math.max(1, v - 1));
                    }}
                    className="inline-flex items-center justify-center rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm font-medium text-zinc-900 hover:bg-zinc-50 disabled:opacity-50"
                  >
                    Remove Last Block
